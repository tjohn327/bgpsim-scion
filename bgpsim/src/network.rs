...
impl<P: Prefix, Q: EventQueue<P>, Ospf: OspfImpl> Network<P, Q, Ospf> {
    ...
    // ==================== SCION Beaconing APIs ====================
    //
    // Two APIs are available for SCION beaconing simulation:
    //
    // ## 1. Event-Based API (Realistic, Flexible)
    //
    // Uses `handle_scion_event()` with `ScionEvent` types:
    // - `BeaconTimeout`: Triggers an AS to propagate PCBs to neighbors
    // - `BeaconBatch`: Delivers PCBs from one AS to another
    // - `RegistrationTimeout`: Triggers segment registration
    // - `SegmentRegistration`: Delivers segments to core ASes
    //
    // **Usage pattern:**
    // ```
    // // Trigger beaconing for one AS
    // let event = Event::scion((), src_as, src_as, ScionEvent::BeaconTimeout { ... });
    // let (_, new_events) = net.handle_scion_event(event)?;
    //
    // // Process resulting BeaconBatch events
    // for event in new_events {
    //     net.handle_scion_event(event)?;
    // }
    // ```
    //
    // **When to use:**
    // - When you need fine-grained control over event ordering
    // - When simulating realistic timing (with custom EventQueue)
    // - When debugging or tracing individual beacon propagation
    // - For small-scale simulations (<100 ASes)
    //
    // ## 2. Batch API (Efficient, Large-Scale)
    //
    // Uses `propagate_intra_isd_batch()` and `propagate_core_batch()`:
    // - Propagates for multiple ASes in a single call
    // - Groups PCBs by destination (fewer HashMap lookups)
    // - No Event object allocation overhead
    // - Returns set of ASes that received beacons
    //
    // **Usage pattern:**
    // ```
    // // Intra-ISD: propagate level-by-level
    ...
    // }
    //
    // // Core: propagate for O(diameter) rounds
    ...
    //
    // **When to use:**
    // - For large-scale simulations (100s-1000s of ASes)
    // - When event ordering within a "round" doesn't matter
    // - When performance is critical
    // - For batch testing and benchmarking
    //
    // ## Performance Comparison (1000 ASes, 3-level hierarchy)
    //
    // | Metric              | Event-Based    | Batch API      |
    // |---------------------|----------------|----------------|
    // | Event allocations   | ~3000          | 0              |
    // | HashMap lookups     | ~6000          | ~2000          |
    // | Function calls      | ~3000          | ~3             |
    // | Code complexity     | Higher         | Lower          |
    //
    // ================================================================

    /// Handle a SCION event at the AS level (Event-Based API)
    ///
    ...
    /// # Event Types
    ///
    /// - [`ScionEvent::BeaconTimeout`]: Triggers PCB propagation from an AS
    /// - [`ScionEvent::BeaconBatch`]: Delivers PCBs to a destination AS
    /// - [`ScionEvent::RegistrationTimeout`]: Triggers segment registration
    /// - [`ScionEvent::SegmentRegistration`]: Delivers segments to core AS
    ///
    /// # Returns
    ///
    /// Returns `(StepUpdate, Vec<Event>)` where the vector contains new events
    /// generated by this event (e.g., BeaconBatch events from a BeaconTimeout).
    ///
    /// # Example
    ///
    /// ```ignore
    /// // Trigger intra-ISD beaconing for a core AS
    /// let event = Event::scion((), core_as, core_as, ScionEvent::BeaconTimeout {
    ///     interval_type: BeaconIntervalType::IntraIsd,
    /// });
    /// let (_, new_events) = net.handle_scion_event(event)?;
    ///
    /// // Process the resulting BeaconBatch events
    /// for event in new_events {
    ///     net.handle_scion_event(event)?;
    /// }
    /// ```
    ...
    /// # Performance Note
    ///
    /// For large-scale simulations, consider using the batch APIs
    /// (`propagate_intra_isd_batch`, `propagate_core_batch`) instead,
    /// which avoid per-event overhead.
    ...
    /// Batch intra-ISD propagation for multiple ASes at once (Batch API)
    ///
    /// Propagates PCBs from all source ASes to their children in a single operation.
    /// This is the recommended API for large-scale simulations.
    ///
    /// # Efficiency
    ///
    /// This method is more efficient than individual `BeaconTimeout` events:
    /// - **No Event allocation**: PCBs are transferred directly
    /// - **Grouped by destination**: Single HashMap lookup per destination AS
    /// - **No queue overhead**: Bypasses the event queue entirely
    ///
    /// # Arguments
    ///
    /// * `source_ases` - Slice of ASes that should propagate PCBs to their children
    ///
    /// # Returns
    ///
    /// Returns a `HashSet` of ASes that received new beacons. Use this to determine
    /// the next level of propagation.
    ///
    /// # Example
    ///
    /// ```ignore
    /// // Propagate intra-ISD beacons level-by-level (Core -> Intermediate -> Leaf)
    /// let mut current_level: Vec<IsdAs> = core_ases.clone();
    ///
    /// while !current_level.is_empty() {
    ///     let next_level = net.propagate_intra_isd_batch(&current_level)?;
    ///     current_level = next_level.into_iter().collect();
    /// }
    /// // All ASes now have up-segments in their beacon stores
    /// ```
    ///
    /// # Beaconing Flow
    ///
    /// Intra-ISD beaconing flows **top-down** through the ISD hierarchy:
    /// 1. Core ASes originate PCBs
    /// 2. Non-core ASes receive, extend, and propagate to children
    /// 3. Process continues until leaf ASes (no children)
    ///
    /// Each AS only needs ONE propagation call - the hierarchy is traversed
    /// in O(depth) rounds, not O(ASes) rounds.
    pub fn propagate_intra_isd_batch(
        &mut self,
        source_ases: &[crate::scion::IsdAs],
    ) -> Result<std::collections::HashSet<crate::scion::IsdAs>, NetworkError> {
        use crate::scion::{IsdAs, ScionLinkType};
        ...
        for &src_as in source_ases {
            ...
            // Get PCBs to propagate to each child
            let batches = control_service.propagate_intra_isd_pcbs();

            for (remote_as, pcbs) in batches {
                by_destination.entry(remote_as).or_default().extend(pcbs);
            }
        }
        ...
        for (dst_as, pcbs) in by_destination {
            ...
            // Validate and store PCBs
            for pcb in pcbs {
                if control_service.validate_pcb(&pcb, ScionLinkType::Parent, None) {
                    control_service.beacon_store.insert(pcb);
                }
            }

            receivers.insert(dst_as);
        }

        Ok(receivers)
    }

    /// Batch core propagation for multiple core ASes at once (Batch API)
    ///
    /// Propagates PCBs between core ASes in a single operation. Core beaconing
    /// is omnidirectional (mesh), unlike intra-ISD which is top-down.
    ///
    /// # Efficiency
    ///
    /// Same benefits as `propagate_intra_isd_batch`:
    /// - No Event allocation
    ...
    /// # Arguments
    ///
    /// * `core_ases` - Slice of core ASes that should propagate PCBs
    ///
    /// # Returns
    ///
    /// Returns a `HashSet` of core ASes that received new beacons.
    ///
    /// # Example
    ///
    /// ```ignore
    /// // Core beaconing requires O(diameter) rounds for full mesh propagation
    /// // In practice, num_cores rounds is sufficient for any mesh topology
    /// for _ in 0..core_ases.len() {
    ///     net.propagate_core_batch(&core_ases)?;
    /// }
    /// // All core ASes now have PCBs from all other reachable cores
    /// ```
    ///
    /// # Beaconing Flow
    ///
    /// Core beaconing is **omnidirectional** through the core mesh:
    /// 1. Round 0: Each core originates PCBs (beacon store empty)
    /// 2. Round 1+: Each core propagates received PCBs to neighbors
    /// 3. After O(diameter) rounds, all cores have paths to all other cores
    ///
    /// Unlike intra-ISD, core beaconing requires multiple rounds because
    /// the core topology is a mesh, not a tree.
    pub fn propagate_core_batch(
        &mut self,
        core_ases: &[crate::scion::IsdAs],
    ) -> Result<std::collections::HashSet<crate::scion::IsdAs>, NetworkError> {
        use crate::scion::{IsdAs, ScionLinkType};
        use std::collections::HashMap;
        use std::sync::Arc;
        ...
        for &src_as in core_ases {
            ...
            // Get PCBs to propagate to each core neighbor
            let batches = control_service.propagate_core_pcbs();
        ...
        for (dst_as, pcbs) in by_destination {
            ...
            // Validate and store PCBs
            for pcb in pcbs {
                if control_service.validate_pcb(&pcb, ScionLinkType::Core, None) {
                    control_service.beacon_store.insert(pcb);
                }
            }
